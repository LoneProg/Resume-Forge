...
import { InstagramIcon, LinkedinIcon, TwitterIcon, YoutubeIcon,HeartHandshake } from "lucide-react";
    ...
    return (
        <footer className="flex flex-wrap justify-center md:justify-between overflow-hidden gap-10 md:gap-20 mt-40 py-6 px-6 md:px-16 lg:px-24 xl:px-32 text-[13px] text-gray-500">
            ...
            <motion.div className="flex flex-col max-md:items-center max-md:text-center gap-2 items-end"
                ...
                transition={{ type: "spring", stiffness: 280, damping: 70, mass: 1 }}
            >
                <p className="max-w-60">  Built with 💜.</p>
                <div className="flex items-center gap-4 mt-3">
                    <a href="https://dribbble.com/prebuiltui" target="_blank" rel="noreferrer">
                        <InstagramIcon className="size-5 hover:text-purple-500" />
                    </a>
                    ...
                    <a href="https://www.youtube.com/@davidcody" target="_blank" rel="noreferrer">
                        <YoutubeIcon className="size-6 hover:text-purple-500" />
                    </a>
                </div>
                <p className="mt-3 text-center">&copy; {new Date().getFullYear()} <a href="#">Cvify</a></p>
            </motion.div>
        </footer>
    );
}